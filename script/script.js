...
const factorCostExp = [2, 2, 2, 3, 3, 6, 8, 10, 15, 21, 28, 36, 45, 55, 66, 78, 91, 105];
...
function loop(unadjusted, off = 0) {
  ...
  game.boosters =
    (game.factorBoosts + Math.round(Math.max(getSumOC(),1)*(calcSlugMile() + getBaseless()))) -
    calcRefund();
  ...
  game.autoLoop.succ/=qfactorMult;
  game.autoLoop.lim/=qfactorMult;
  //test ^
  game.autoLoop.succ/=game.achievement.length;
  game.autoLoop.lim/=game.achievement.length;
...
They are based on your Singularity level.`
  ...
  get("bup9Cost").textContent=Math.round(64**(0.5**getOCComp(3)))
