...
function project(x) {
  if (game.OP >= V(game.factorBoosts + 3, 1) && game.bulkBoost == 0) {
    ...
    if(game.ord<1e270){
	    ...
	    let goal = factorShiftCosts[game.factorShifts]/OPmult
	    ...
	    get("nextBulkTime").textContent=percent==100?"Goal reached!":"The next Factor Shift will take " + time((ordGoal-ord)/(rate)) + " assuming your Tier 1 speed stays constant"
	}else{
		get("factorBoostProg").style.width="100%"
	    get("factorBoostProg").textContent="100.00%"
	    get("nextBulkTime").textContent="Get your Factor Shift 8!"
		/*
	    get("nextBulkTime").innerHTML =
